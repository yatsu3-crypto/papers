...
成果物は `papers.html`（リポジトリ直下・単一 HTML）への **データ追記** だけ。見た目は `staff` リポジトリの washi-design（和紙デザインシステム）に従っており、HTML の骨格はいじらない。
...
| `scripts/iphone-shot.sh` | `iphone-shot.sh papers.html 390 1400` — Chrome ヘッドレスで iPhone 幅の描画を撮る（Mac のみ） |
...
{ cat:"A",                       // "A" | "B" | "C"
  ...
  sum:["何を調べたか（対象・方法・規模）",
       "結果（主要な数値をそのまま）",
       "臨床的示唆（著者の結論の範囲で）"],
  use:"当院の診療でどう使えるか（一文。院内アプリ「生活指導アシスタント」の生活指導・当院製品・外用薬・パッチ・食物アレルギー等のタブと結びつける）",
...
.claude/skills/paper-digest/scripts/iphone-shot.sh papers.html 390 1400   # 撮って Read で目視（Google Chrome がある Mac のみ。クラウド実行では省略）
